...
def save_checkpoint(exp_name, epoch, tm, best_val_mcc):
    ...
    checkpoint_path = f'D:/{project_dir}/perf/{exp_name}/checkpoint_epoch_{epoch}.pkl'
    state_dict = tm.save()
    checkpoint = {
        'epoch': epoch,
    ...
    with open(f'D:/{project_dir}/perf/{exp_name}/latest_checkpoint.txt', 'w') as f:
        f.write(str(epoch))
...
def load_checkpoint(exp_name):
    ...
    try:
        with open(f'D:/{project_dir}/perf/{exp_name}/latest_checkpoint.txt', 'r') as f:
            last_epoch = int(f.read().strip())
        
        checkpoint_path = f'D:/{project_dir}/perf/{exp_name}/checkpoint_epoch_{last_epoch}.pkl'
...
def objective(trial, X_train_optuna, Y_train_optuna, X_val_optuna, Y_val_optuna, x_dim, y_dim, exp_name, epochs=30):
    # Suggest hyperparameters
    number_of_clauses = trial.suggest_int('number_of_clauses', 1000, 10000, step=1000)
    T = trial.suggest_categorical('T', [100, 1000, 10000])
    s = trial.suggest_categorical('s', [1.4])
    patch_dim_size = trial.suggest_int('patch_dim_size', 5, 30, step=5)
    n_bits = trial.suggest_categorical('n_bits', [8, 16, 32])
    
    # Binarize with the suggested n_bits for this trial
    X_train_trial_bin = binarize_images(X_train_optuna, n_bits).reshape(X_train_optuna.shape[0], -1).astype(np.uint32)
    X_val_trial_bin = binarize_images(X_val_optuna, n_bits).reshape(X_val_optuna.shape[0], -1).astype(np.uint32)
    ...
    batch_size = 1024
    n_samples = X_train_trial_bin.shape[0]
    
    for i in range(epochs):
        seg = (i % ((n_samples + batch_size - 1) // batch_size)) * batch_size
        end_seg = min(seg + batch_size, n_samples)
        
        X_train_seg = X_train_trial_bin[seg:end_seg]
        Y_train_seg = Y_train_optuna[seg:end_seg]
        
        tm.fit(X_train_seg, Y_train_seg, epochs=1, incremental=True)
        
        y_pred = tm.predict(X_val_trial_bin)
        metrics = calculate_metrics(Y_val_optuna, y_pred)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--optuna_epochs", default=30, type=int, help="Epochs per trial")
    parser.add_argument("--final_epochs", default=60, type=int, help="Epochs for final training")
    parser.add_argument("--checkpoint_freq", default=5, type=int, help="Save checkpoint every N epochs")
    args = parser.parse_args()
    
    exp_name = args.exp_name
    print(f"Experiment Name: {exp_name}")
    project_dir = 'path_to_project_folder'                                  # Change this to your actual project directory
    os.makedirs(f'{project_dir}/perf/{exp_name}', exist_ok=True)

    dataset_path = 'path_to_dataset/dataset.h5'                             # Change this to your actual dataset path

    # Loading the dataset - use only ONE fold for Optuna optimization
    print("Loading data for Optuna optimization (using single fold)...")
    with h5py.File(dataset_path, 'r') as f:
        X_train_optuna = f['fold_0']['image'][:]
        Y_train_optuna = f['fold_0']['target'][:]
        X_val_optuna = f['fold_1']['image'][:]
        Y_val_optuna = f['fold_1']['target'][:]

    if len(X_train_optuna.shape) == 3:
        X_train_optuna = X_train_optuna[..., np.newaxis]
        X_val_optuna = X_val_optuna[..., np.newaxis]

    _, x_dim, y_dim, __ = X_train_optuna.shape

    # Check if resuming or starting fresh
    if args.resume:
        # Load existing study
        study_path = f'{project_dir}/perf/{exp_name}/optuna_study.db'
        if os.path.exists(study_path):
            ...
            print(f"Resuming Optuna optimization from trial {len(study.trials)}...")
        else:
            print("No existing study found, starting new optimization...")
            study = optuna.create_study(
                study_name=exp_name,
                direction='maximize',
                storage=f'sqlite:///{project_dir}/perf/{exp_name}/optuna_study.db',
                pruner=optuna.pruners.MedianPruner(n_startup_trials=5, n_warmup_steps=10),
                load_if_exists=True
            )
    else:
        # Create new study with persistent storage
    ...
    remaining_trials = args.n_trials - len(study.trials)
    if remaining_trials > 0:
        study.optimize(
            lambda trial: objective(trial, X_train_optuna, Y_train_optuna, X_val_optuna, Y_val_optuna, 
                                    x_dim, y_dim, exp_name, epochs=args.optuna_epochs),
            n_trials=remaining_trials
        )
    else:
        print(f"Optuna optimization already completed ({len(study.trials)} trials)")

    # Save Optuna results
    print("\n=== Optuna Optimization Results ===")
    print(f"Best trial: {study.best_trial.number}")
    print(f"Best MCC: {study.best_trial.value:.4f}")
    print(f"Best parameters: {study.best_params}")
    
    with open(f'{project_dir}/perf/{exp_name}/optuna_results.json', 'w') as f:
        json.dump({
    ...
    n_bits = study.best_params['n_bits']
    ...
    best_configs = {
        'number_of_clauses': study.best_params['number_of_clauses'],
        'T': study.best_params['T'],
        's': study.best_params['s'],
        'dim': (x_dim, y_dim, n_bits),
        'patch_dim': (study.best_params['patch_dim_size'], study.best_params['patch_dim_size']),
    }
    
    with open(f'{project_dir}/perf/{exp_name}/best_configs.json', 'w') as f:
        info = {'dataset_path': dataset_path, 'n_bits': n_bits}
        info.update(best_configs)
    ...
    tm = MultiClassConvolutionalTsetlinMachine2D(**best_configs)
    ...
    for i in range(start_epoch, args.final_epochs):
        batch_size = 1024
    ...
    new_tm = MultiClassConvolutionalTsetlinMachine2D(**best_configs)
